...
from src.retrieval_graph.researcher_graph.graph import Researcher_graph as researcher_graph

from src.retrieval_graph.state import AgentState, InputState, Router #(used to route queries based on being relevant or general)
...
def route_query(
  ...
  """
   ...
   Args:
    ...
    Returns:
      Literal["create_research_plan", "ask_for_more_info", "respond_to_general_query"]:
      The next step to take based on the classification.
...
async def ask_for_more_info(
  ...
  Args:
    ...
    config (RunnableConfig): Configuration with the model used to query
...
async def respond_to_general_query(
  ...
  """
  Generate a response to a general query not related to the selected climate regulation.

  This node is called when the router classifies the query as a general question.

  Args:
    state (AgentState): The current state of the agent, including conversation histroy and router logic.
...
async def create_research_plan(
  ...
  """
  Create a step-by-step research plan for answering a selected climate regulation related query.

  Args:
    state (AgentState): The current state of the agent, including conversation history.
    config (RunnableConfig): Configuration with the model used to generate the research plan.
...
#   return {"messages": [response]}
  ...
  context = format_docs(state.documents)
    ...
    # including the page numeber in the final response

  metadata_string = ""
  for i, doc in enumerate(state.documents):  
    page_number = doc.metadata.get("page_label", "N/A")
        # print(f"page_Number : {page_number}")
    metadata_string += f"Page Number : {page_number}\n"
        # source = doc.metadata.get("source", "N/A")
        # metadata_string += f"[{i + 1}] Source: {source}, Page: {page_number}\n"
